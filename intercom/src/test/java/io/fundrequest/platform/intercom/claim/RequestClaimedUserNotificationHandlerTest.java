...
class RequestClaimedUserNotificationHandlerTest {
    ...
    void handle() {
        ...
        when(identityAPIClient.findByIdentityProviderAndFederatedUsername(requestDto.getIssueInformation().getPlatform().name(), solver)).thenReturn(userRepresentation);
