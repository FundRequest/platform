...
class NotificationServiceImpl implements NotificationService {
    ...
    private List<NotificationDto> getRequestFundedNotifications(final List<Notification> notifications) {
        final List<RequestFundedNotification> requestFundedNotifications = notifications.stream()
                                                                                        .filter(n -> n.getType() == NotificationType.REQUEST_FUNDED)
                                                                                        .map(n -> (RequestFundedNotification) n)
                                                                                        .collect(Collectors.toList());

        final Map<Long, FundDto> fundedMap = fundService.findAll(requestFundedNotifications.stream().map(RequestFundedNotification::getFundId).collect(Collectors.toSet()))
                                                        .stream()
                                                        .collect(Collectors.toMap(FundDto::getId, n -> n));

        final Map<Long, RequestDto> requestMap = requestService.findAll(fundedMap.values().stream().map(FundDto::getRequestId).collect(Collectors.toSet()))
                                                               .stream()
                                                               .collect(Collectors.toMap(RequestDto::getId, n -> n));
