...
class NotificationServiceImpl implements NotificationService {
    ...
    private List<NotificationDto> getRequestClaimedNotifications(List<Notification> notifications) {
        ...
        return requestCreatedNotifications.stream()
                .map(n -> new RequestClaimedNotificationDto(n.getId(), n.getDate(), requestMap.get(n.getRequestId()), n.getSolver()))
                .collect(Collectors.toList());
    }
    ...
    public void onFunded(RequestFundedEvent fundedEvent) {
        RequestFundedNotification notification = new RequestFundedNotification(NotificationType.REQUEST_FUNDED, fundedEvent.getTimestamp(), fundedEvent.getFundDto().getId());
    ...
    public void onClaimed(RequestClaimedEvent claimedEvent) {
        RequestClaimedNotification notification = new RequestClaimedNotification(NotificationType.REQUEST_CLAIMED, claimedEvent.getTimestamp(), claimedEvent.getRequestDto().getId(), claimedEvent.getSolver());
        notification = notificationRepository.saveAndFlush(notification);
        publishNotification(
    ...
    private RequestClaimedNotificationDto createRequestClaimedNotification(Long id, RequestClaimedEvent claimedEvent) {
        return new RequestClaimedNotificationDto(id, claimedEvent.getTimestamp(), claimedEvent.getRequestDto(), claimedEvent.getSolver());
    }
