define(['require', 'jquery'], function (require, $) {
  ...
  function fundIssue(issueId, value, callback) {
    var total = web3.toWei(value, 'ether');
    tokenContract.transferFunding(total, '' + issueId, function (err, result) {
      callback(err, result);
    });
  }
  ...
  function getIssueBalance(issueId, callback) {
    fundRequestContract.balance.call('' + issueId, function (err, result) {
      var balance;
      if (result) {
        balance = web3.fromWei(result.toString(10));
      }
  ...
  return {
    ...
    fundIssue: fundIssue,
    getIssueBalance: getIssueBalance
  }
